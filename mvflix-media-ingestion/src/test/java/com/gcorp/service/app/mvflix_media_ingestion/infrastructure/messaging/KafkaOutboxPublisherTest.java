...
class KafkaOutboxPublisherTest {
  @Test
  void publishesEachEventToItsDurableTopic() {
    KafkaTemplate<String, String> kafka = mock(KafkaTemplate.class);
    when(kafka.send(eq("mvflix.media-ingestion-completed.v1"), anyString(), anyString()))
        .thenReturn(CompletableFuture.completedFuture(null));
    var message =
        new Message(
            UUID.randomUUID(), "MediaIngestionCompleted", UUID.randomUUID(), "{}", Instant.now());
