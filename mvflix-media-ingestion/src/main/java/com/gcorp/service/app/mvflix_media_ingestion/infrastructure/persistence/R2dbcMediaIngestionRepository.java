...
public class R2dbcMediaIngestionRepository implements MediaIngestionRepository {
  ...
  public Mono<Boolean> compareAndSet(MediaIngestion e,MediaIngestion n) { var s=db.sql("UPDATE media_ingestions SET catalog_item_id=:c,upload_id=:u,storage_id=:sid,storage_key=:skey,upload_url=:url,phase=:p,failure_code=:f,version=:nv,updated_at=:now,retry_count=:r,next_attempt_at=:next WHERE ingestion_id=:id AND version=:ov"); s=bind(s,"c",n.catalogItemId(),Long.class); s=bind(s,"u",n.uploadId(),String.class); s=bind(s,"sid",n.storageId(),Long.class); s=bind(s,"skey",n.storageKey(),String.class); s=bind(s,"url",n.uploadUrl(),String.class); s=bind(s,"p",n.phase().name(),String.class); s=bind(s,"f",n.failureCode(),String.class); s=bind(s,"nv",n.version(),Long.class); s=bind(s,"now",n.updatedAt(),Instant.class); s=bind(s,"r",n.retryCount(),Integer.class); s=bind(s,"next",n.nextAttemptAt(),Instant.class); return bind(s,"id",e.ingestionId(),UUID.class).bind("ov",e.version()).fetch().rowsUpdated().map(x->x==1); }
}
